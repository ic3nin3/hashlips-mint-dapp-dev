...
function App() {
  ...
  const claimWLNFTs = () => {
    ...
    console.log("Gas limit: ", totalGasLimit);
    setFeedback(`Minting your ${CONFIG.NFT_NAME}...`);
    const isContained = CONFIG.ALLOW_LIST.some(element => {
      return element.toLowerCase() === blockchain.account.toLowerCase();
    });
    if(!isContained){      
      console.log("You Are Not Whitelisted");
      setFeedback(`You Are Not Whitelisted`);            
    }
    else{
      setClaimingNft(true);
      blockchain.smartContract.methods
